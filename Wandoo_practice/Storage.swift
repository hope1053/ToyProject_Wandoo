...
// MARK: TEST 용
extension Storage {
    static func saveLec(_ obj: LectureInfo, fileName: String) {
...
//        print("---> [TEST] save to here: \(url)")
    ...
    static func restoreLec(_ fileName: String) -> LectureInfo? {
        let url = Directory.documents.url.appendingPathComponent(fileName, isDirectory: false)
        guard FileManager.default.fileExists(atPath: url.path) else { return nil }
        guard let data = FileManager.default.contents(atPath: url.path) else { return nil }
        ...
        do {
            let model = try decoder.decode(LectureInfo.self, from: data)
            return model
        } catch let error {
            print("---> Failed to decode msg: \(error.localizedDescription)")
            return nil
